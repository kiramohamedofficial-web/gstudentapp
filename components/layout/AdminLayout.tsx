...
const navItems = [
    ...
    { id: 'questionBank', label: 'الأسئلة والاستفسارات', icon: QuestionMarkCircleIcon },
    { id: 'tools', label: 'الأدوات', icon: QrcodeIcon },
    { id: 'platformSettings', label: 'الإعدادات', icon: CogIcon },
];
...
const NavButton: React.FC<{
    ...
    <button
        onClick={onClick}
        className={`relative w-full text-right flex items-center space-x-4 space-x-reverse transition-all duration-300 group rounded-lg p-3 nav-btn admin
        ${isActive
            ? 'active'
            : 'text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)] hover:text-[var(--text-primary)]'}`}
    >
        <Icon className={`w-6 h-6 transition-transform duration-300 ${isActive ? 'text-purple-500' : 'group-hover:scale-110 text-gray-500'}`} />
        <span className="text-md">{label}</span>
    </button>
);


const NavContent: React.FC<{ activeView: string; onNavClick: (view: AdminView) => void; onLogout: () => void; }> = ({ activeView, onNavClick, onLogout }) => (
