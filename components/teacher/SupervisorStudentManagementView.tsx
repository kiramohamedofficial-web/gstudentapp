...
import { User, Grade, Subscription, QuizAttempt, Teacher } from '../../types';
...
const SupervisorStudentManagementView: React.FC<SupervisorStudentManagementViewProps> = ({ supervisedTeachers, onViewDetails }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            const supervisedTeacherIds = new Set(supervisedTeachers.map(t => t.id));
            ...
            const relevantSubs = subs.filter(s => s.teacherId && supervisedTeacherIds.has(s.teacherId));
            const relevantStudentIds = new Set(relevantSubs.map(s => s.userId));

            setAllGrades(grades);
            setAllUsers(users.filter(u => u.role === 'student' && relevantStudentIds.has(u.id)));
            setSubscriptions(new Map(relevantSubs.map(s => [s.userId, s])));
                                ...
                                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 gap-4">
                                    ...
                                    <div className="flex items-center gap-4 flex-shrink-0 self-start sm:self-center">
                                        ...
                                        <button onClick={() => onViewDetails(user)} className="py-2 px-4 text-sm font-semibold text-purple-300 bg-purple-600/20 hover:bg-purple-600/40 rounded-lg transition-colors whitespace-nowrap flex items-center">
                                            عرض التفاصيل <ChevronLeftIcon className="w-4 h-4 mr-1" />
                                        </button>
                                    </div>
                                </div>
